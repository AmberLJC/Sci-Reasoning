#!/usr/bin/env python3
"""
Research Idea Generation Evaluation - Gemini 3 Pro Preview
Uses Exa AI for paper retrieval, Gemini for idea generation and judging
"""
...
# Pricing for Gemini 3 Pro (estimate - adjust as needed)
    ...
    for i, pred_title in enumerate(paper_data['predecessors']):
        ...
        if result["success"]:
            ...
            if quality in ["good", "good_from_html"]:
                quality_content += 1
            print(f"        ✓ Found: {result.get('url', 'N/A')[:60]} ({len(result.get('content', ''))} chars)")
        else:
...
def save_results(results, total_input_tokens, total_output_tokens, start_time, interim=False):
    ...
    output_data = {
        "summary": {"model": "gemini-3-pro-preview", "k": 10, "crawl_method": "exa_ai",
    ...
    output_path = f"projects/research_idea_evaluation/results/evaluation_results_gemini_pro{suffix}.json"
...
def main():
    print("="*70)
    print("Research Idea Generation - Gemini 3 Pro Preview")
    ...
    print("\n" + "="*70)
    print("FINAL RESULTS - Gemini 3 Pro Preview")
    print("="*70)
