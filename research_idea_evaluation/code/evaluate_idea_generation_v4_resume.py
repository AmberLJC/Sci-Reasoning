#!/usr/bin/env python3
"""
Research Idea Generation Evaluation Pipeline v4 - RESUME
Resumes from paper 35 where we left off
"""
...
def evaluate_single_paper(paper_data, paper_idx, k=10):
    ...
    for i, idea in enumerate(ideas):
        is_match, in_tokens, out_tokens = judge_similarity(
            idea, paper_data["paper_title"], paper_data["contribution"]
        )
        total_input_tokens += in_tokens
        total_output_tokens += out_tokens
...
def main():
    print("="*70)
    print("Research Idea Generation Evaluation v4 - RESUME")
    print("="*70)
    ...
    total_input_tokens = prev_data['summary']['cost']['input_tokens']
    total_output_tokens = prev_data['summary']['cost']['output_tokens']
    
    print(f"Loaded {len(results)} previous results")
    print(f"Previous tokens: {total_input_tokens:,} in / {total_output_tokens:,} out")
    ...
    for idx in range(RESUME_FROM, len(papers)):
        ...
        print(f"\n    Progress: {idx+1}/{len(papers)} | Hits: {hits_so_far}/{len(results)} ({hits_so_far/len(results)*100:.1f}%)")
    ...
    print(f"Resume runtime: {elapsed/60:.1f} minutes")
    print(f"\nTotal API Cost:")
