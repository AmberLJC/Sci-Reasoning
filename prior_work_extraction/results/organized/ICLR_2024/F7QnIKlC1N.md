...
**Abstract:** 
> The ground-state conformation of a molecule is often decisive for its properties. However, experimental or computational methods, such as density functional theory (DFT), are time-consuming and labor-intensive for obtaining this conformation. Deep learning (DL) based molecular representation learning (MRL) has made significant advancements in molecular modeling and has achieved remarkable results in various tasks. Consequently, it has emerged as a promising approach for directly predicting the g...
...
- *Direct Connection:* EGNN established a strong coordinate-update paradigm but its reliance on strict equivariance and localized message passing highlighted a limitation in modeling long-range constraints that GTMGC addresses with global transformer attention via MSRSA.

**SE(3)-Transformer: 3D Roto-Translation Equivariant Attention Networks** (2020)
- *Authors:* Fabian Fuchs et al.
- *Direct Connection:* SE(3)-Transformer showed attention can be made equivariant for 3D structure but with high computational/tensor complexity, motivating GTMGC’s lightweight MSRSA that injects structural biases into attention without expensive equivariant operations.

### 📊 Baseline

**GeoDiff: A Geometric Diffusion Model for Molecular Conformation Generation** (2022)
- *Authors:* Minkai Xu et al.
- *Direct Connection:* As a leading iterative diffusion method for conformer generation, GeoDiff serves as a primary baseline GTMGC improves upon by predicting the ground-state conformation in a single forward pass instead of costly sampling.

**Better Informed Distance Geometry: Using What We Know To Improve Conformation Generation (ETKDG)** (2015)
- *Authors:* Sereina Riniker et al.
- *Direct Connection:* ETKDG provides the classical heuristic baseline for conformer generation that GTMGC directly replaces with a learned, end-to-end Graph-Transformer that targets DFT-quality ground-state geometries.

### 🔧 Extension

**Do Transformers Really Perform Badly for Graph Representation?** (2021)
- *Authors:* Chengxuan Ying et al.
- *Direct Connection:* GTMGC extends Graphormer’s idea of injecting structural biases into attention by designing Molecule Structural Residual Self-Attention (MSRSA), which adds molecule-specific structural residuals (e.g., bond/path/ring cues) directly to self-attention to guide 2D-to-3D prediction.
