...
- *Connection:* BEATs adopts HuBERT’s core idea of predicting offline discovered discrete units under a masked modeling objective and extends HuBERT’s iterative target refinement into an explicit iterative co-optimization of an acoustic tokenizer and the SSL model for general audio.

**wav2vec 2.0: A Framework for Self-Supervised Learning of Speech Representations** (2020)
- *Authors:* Alexei Baevski et al.
- *Connection:* By showing that learning to predict quantized discrete targets yields strong speech representations without reconstruction, wav2vec 2.0 directly motivates BEATs’ choice of discrete label prediction rather than spectrogram reconstruction.

### 💡 Inspiration

**vq-wav2vec: Self-Supervised Learning of Discrete Speech Representations** (2019)
- *Authors:* Alexei Baevski et al.
- *Connection:* vq-wav2vec introduced learning discrete acoustic units via vector quantization for use as prediction targets, a mechanism BEATs generalizes by learning a semantic-rich acoustic tokenizer whose codes become the labels for masked prediction.

**BEiT: BERT Pre-Training of Image Transformers** (2021)
- *Authors:* Hangbo Bao et al.
- *Connection:* BEiT pioneered masked modeling using discrete labels produced by an external tokenizer; BEATs transfers this paradigm to audio by predicting tokens from an acoustic tokenizer instead of reconstructing spectrograms.

### 📊 Baseline

**SSAST: Self-Supervised Audio Spectrogram Transformer** (2021)
- *Authors:* Yuan Gong et al.
- *Connection:* As a leading reconstruction-based audio SSL method (masked spectrogram modeling), SSAST serves as a primary baseline whose limitation—overemphasis on low-level reconstruction—BEATs addresses by predicting semantic discrete labels from an acoustic tokenizer.

### 🔧 Extension

**BEiT v2: Masked Image Modeling with Vector-Quantized Visual Tokenizers** (2022)
- *Authors:* Hangbo Bao et al.
- *Connection:* BEiT v2’s emphasis on improving and aligning the tokenizer to yield semantically meaningful targets directly informs BEATs’ iterative pipeline that jointly optimizes a semantic-rich acoustic tokenizer and the audio SSL model.

**WavLM: Large-Scale Self-Supervised Pre-Training for Full Stack Speech Processing** (2022)
- *Authors:* Sanyuan Chen et al.
- *Connection:* WavLM extends HuBERT-style masked unit prediction and provides strong semantic speech representations; BEATs leverages this lineage to bootstrap semantic acoustic tokens and extends the approach from speech to general audio events via iterative tokenizer-model refinement.
...
---
*Generated: 2026-01-06T23:09:26.573879*
